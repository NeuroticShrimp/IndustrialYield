...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(`https://financialmodelingprep.com/stable/earnings?symbol=${symbol}&apikey=${API_KEY}`)
    ...
    return NextResponse.json({ error: "Failed to fetch earnings data" }, { status: 500, headers: corsHeaders(origin) })
  }
}
