...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(
      `https://financialmodelingprep.com/stable/dividends?symbol=${symbol}&apikey=${API_KEY}`,
    )
    ...
    return NextResponse.json({ error: "Failed to fetch dividend data" }, { status: 500, headers: corsHeaders(origin) })
  }
}
